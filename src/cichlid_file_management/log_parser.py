        ...
        with open(self.logfile) as f:
            for line in f:
                ...
                if info_type == 'PiCameraStopped':
                    ...
                    # The tracker now writes MP4 filenames; match either the mp4
                    # or the legacy h264 name so end times are always recorded.
                    matches = [x for x in self.movies if t_list[1] in (x.mp4_file, x.h264_file)]
                    if matches:
...
class Trial:
    def __init__(self, start_time, stop_time, reset_time, all_frames, all_movies, sampleID):
        ...
        # First frame after the tank reset, if any (used by downstream summaries).
        self.reset_frame = None
        if reset_time is not None:
            after_reset = [x for x in all_frames if x.time > reset_time]
            self.reset_frame = after_reset[0] if after_reset else None

        self.frames = [x for x in all_frames if start_time < x.time < stop_time]
        self.daylight_frames = [x for x in self.frames if x.lof]

        try:
            self.movies = [x for x in all_movies if x.endTime > start_time and x.startTime < stop_time]
        except TypeError:
            # An unclosed movie has endTime == ''; default it to 18:00 of its start day.
        ...
        days = {}
        if all_frames:
            day_zero = all_frames[-1].time.replace(hour=0, minute=0)
            for frame in self.daylight_frames:
                frame.rel_day = (frame.time - day_zero).days
